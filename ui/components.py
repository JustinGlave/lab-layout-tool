"""Phoenix Controls component helpers — Phase 3A retrofit.

Re-exports the commons widget catalog so existing imports
(``from ui.components import PrimaryButton``, etc.) keep working
without changes. App-specific widgets that don't belong in commons
stay defined locally:

    BackgroundWatermarkWidget    Phoenix-CAD-specific watermark overlay
    WelcomeDialog                first-run orientation modal
    PreferencesDialog            per-user defaults editor
    JobBrowserDialog             project picker

Pre-retrofit: ~640 lines of widget classes, the first ~250 of which
duplicated commons.
Post-retrofit: ~400 lines (commons-duplicate classes removed).

Per PLATFORM_CONTRACT.md § Widgets, app-specific dialogs / sprites
stay app-local; the canonical commons catalog comes via commons
imports. Per COMPONENT_CONTRACT.md the local widgets here are
"composition" examples (they wrap commons primitives into
Phoenix-CAD-specific UX flows).
"""
...
# ── Commons widget catalog (Phase 3A retrofit) ───────────────────────────────
# These were locally defined pre-retrofit; lifted verbatim into
# phoenix_commons.widgets in Phase 2; now re-exported here so existing
# `from ui.components import ...` import sites continue to work without
# any change to app.py / cad/ / ui/main_window.py / ui/pbc.py.
...
# ── Background watermark (app-local — Phoenix-CAD-specific) ───────────────────
...
# ── Welcome dialog (first-run guidance — app-local) ───────────────────────────
...
__all__ = [
    # Commons widget catalog re-exports (Phase 3A retrofit)
