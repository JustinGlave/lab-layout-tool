...
class BlockVariant:
    ...
    # Optional preview image for the variant dropdown. By convention sits
    # next to the DWG with a .png suffix (e.g. SAV_SINGLE.png next to
    # SAV_SINGLE.dwg). Generated by tools/generate_thumbnails.py; if the
    # PNG isn't present, dropdowns just show text — no error.
...
def list_variants(product_line: ProductLine, category: str) -> list[BlockVariant]:
    ...
    for dwg in sorted(cat_dir.glob("*.dwg")):
        thumb = dwg.with_suffix(".png")
        variants.append(
            BlockVariant(
                variant_id=dwg.stem,
                label=_label_for(dwg.stem, category),
                dwg_path=dwg.resolve(),
                thumbnail_path=thumb.resolve() if thumb.is_file() else None,
            )
        )
